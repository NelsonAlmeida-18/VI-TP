...
//             color += specularReflection(isect, f, depth)/continue_p;
//         }
//         else {
//             color += diffuseReflection(isect, f, depth)/continue_p;
//         }
//     }
...
RGB PathTracerShader::specularReflection (Intersection isect, Phong *f, int depth) {
    ...
    // IDEAL SPECULAR REFLECTION
    // direction R = 2 (N.V) N - V
    Vector gn = isect.gn.Faceforward(isect.wo);
    float cos = gn.dot(isect.wo);
    Rdir = 2.f * cos * gn - isect.wo;
    
    Point origin = isect.p + isect.gn;
    Ray specular(origin, Rdir);

    // OK, we have the ray : trace and shade it recursively
    bool intersected;
    // trace ray
    intersected = scene->trace(specular, &isect);
    if (depth>0 && intersected)
        color = shade(intersected, isect, depth-1);
    else if (depth==0 && intersected)
        color = shade(intersected, isect, depth);
...
RGB PathTracerShader::diffuseReflection (Intersection isect, Phong *f, int depth) {
    ...
    float cos_theta = sqrtf(rnd[1]);

    Vector D_around_Z;
    // cosine sampling
    D_around_Z.Y = sinf(2*M_PI*rnd[0])*sqrtf(1.-rnd[1]);
    D_around_Z.X = cosf(2*M_PI*rnd[0])*sqrtf(1.-rnd[1]);
    D_around_Z.Z = cos_theta;
    ...
    //TODO: Verify colision bias
    Ray diffuse(isect.p+isect.gn, D_around_Z.Rotate(Rx,Ry, isect.gn));
        

    // OK, we have the ray : trace and shade it recursively
    bool intersected;
    // trace ray
    intersected = scene->trace(diffuse, &isect);

    if (!isect.isLight) {  // if light source return 0 ; handled by direct
        // shade this intersection
        if (depth>0)
            color = shade(intersected, isect, depth-1);
        else
    ...
    return (f->Kd * cos_theta * color) / pdf;
...
RGB PathTracerShader::shade(bool intersected, Intersection isect, int depth) {
    ...
    if (depth >0 || rnd_russian < continue_p) {
        // Montecarlo sampling 
        float s_p = f->Ks.Y() / (f->Kd.Y() + f->Ks.Y());
        float rnd = ((float)rand()) / ((float)RAND_MAX);
        if (rnd<=s_p || s_p >= (1.-EPSILON)) {
            color += specularReflection(isect, f, depth)/s_p;
        }
        else {
            color += diffuseReflection(isect, f, depth)/(1-s_p);
        }
    }
